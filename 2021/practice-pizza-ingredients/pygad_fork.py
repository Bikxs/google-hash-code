"""
source: https://github.com/ahmedfgad/GeneticAlgorithmPython
GeneticAlgorithmPython: Building Genetic Algorithm in Python
By ahmed.f.gad@gmail.com

Copied it to implement cycle_crossover
...
class GA:
    def __init__(self,
                ...
                for index, el in enumerate(gene_space):
                    if type(el) in [list, tuple, range, numpy.ndarray]:
                        if len(el) == 0:
                            ...
                            for val in el:
                                if not (type(val) in [type(None), int, float, numpy.int, numpy.int8, numpy.int16,
                                                      numpy.int32, numpy.int64, numpy.uint, numpy.uint8, numpy.uint16,
                                                      numpy.uint32, numpy.uint64, numpy.float, numpy.float16,
        ...
        if gene_type in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64, numpy.uint,
                         numpy.uint8, numpy.uint16, numpy.uint32, numpy.uint64, numpy.float, numpy.float16,
                         numpy.float32, numpy.float64]:
            self.gene_type = gene_type
        else:
            self.valid_parameters = False
            raise ValueError(
                "The value passed to the 'gene_type' parameter must be either integer or floating-point number but the value ({gene_type_value}) of type {gene_type_type} found.".format(
                    gene_type_value=gene_type, gene_type_type=type(gene_type)))
        ...
        if (crossover_type == "single_point"):
            ...
            # addition by biko.nyamai@gmail.com
        ...
        elif (mutation_type == "swap2"):
            self.mutation = self.swap_mutation2
        ...
        if not (self.mutation_type is None):
            if mutation_probability is None:
                ...
                if type(mutation_probability) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                  numpy.int64, numpy.uint, numpy.uint8, numpy.uint16, numpy.uint32,
                                                  numpy.uint64, numpy.float, numpy.float16, numpy.float32,
                                                  numpy.float64]:
                    ...
                    if mutation_percent_genes == 'default'.lower():
                        ...
                        if mutation_num_genes == 0:
                            ...
                            mutation_num_genes = 1
                                                          ...
                                                          numpy.int64, numpy.uint, numpy.uint8, numpy.uint16,
                                                          numpy.uint32, numpy.uint64, numpy.float, numpy.float16,
                                                          numpy.float32, numpy.float64]:
                        ...
                        if len(mutation_percent_genes) == 2:
                            ...
                            for idx, el in enumerate(mutation_percent_genes):
                                if type(el) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                numpy.int64, numpy.uint, numpy.uint8, numpy.uint16, numpy.uint32,
                                                numpy.uint64, numpy.float, numpy.float16, numpy.float32, numpy.float64]:
                                    if (el <= 0 or el > 100):
                                        self.valid_parameters = False
                                        raise ValueError(
        ...
        if type(delay_after_gen) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                     numpy.uint, numpy.uint8, numpy.uint16, numpy.uint32, numpy.uint64, numpy.float,
        ...
        if self.gene_space is None:
            ...
            for sol_idx in range(self.sol_per_pop):
                for gene_idx in range(self.num_genes):
                    ...
                    elif type(self.gene_space[gene_idx]) in [int, float, numpy.int, numpy.int8, numpy.int16,
                                                             numpy.int32, numpy.int64, numpy.uint, numpy.uint8,
                                                             numpy.uint16, numpy.uint32, numpy.uint64, numpy.float,
                                                             numpy.float16, numpy.float32, numpy.float64]:
                        self.population[sol_idx, gene_idx] = self.gene_space[gene_idx].copy()
        else:
    ...
    def cycle_crossover(self, parents, offspring_size):
        ...
        It accepts 2 parameters:
            -parents: The parents to mate for producing the offspring.
            -offspring_size: The size of the offspring to produce. Ignored  -offspring will have same size as parents
        It returns an array the produced offspring.
        ...
        offsprings = None
        while True:
            pindexes = random.sample([x for x in range(len(parents))], 2)
            parent0 = numpy.uint32(parents[pindexes[0]]).tolist()
            parent1 = numpy.uint32(parents[pindexes[1]]).tolist()

            # find mismatched DNAs
            in_parent0_not_in_parent1 = list(set(parents[0]) - set(parents[1]))
            in_parent1_not_in_parent0 = list(set(parents[1]) - set(parents[0]))
            ...
            for i in range(2):
                ...
                if i == 0:
                    ...
                    for x in parent0:
                        if x not in cycles[0]:
                            break
                    index_p1 = x
                cycle_start_value = parent0[index_p1]

                while True:
                    value_p2 = parent1[index_p1]
                    cycle.append(index_p1)
                    if parent1[index_p1] in parent0:
                        if value_p2 in parent0:
                            index_p1 = list.index(parent0, value_p2)
                        elif value_p2 in in_parent1_not_in_parent0:
                            index_p1 = list.index(parent0,
                                                  in_parent0_not_in_parent1[
                                                      list.index(in_parent1_not_in_parent0, value_p2)])
                        else:
                            raise Exception("Something weird happen, cycle crossover should be used for permutations")
                    elif parent1[index_p1] in in_parent0_not_in_parent1:
                        index_p1 = list.index(parent1,
                                              in_parent1_not_in_parent0[
                                                  list.index(in_parent0_not_in_parent1, value_p2)])
                    else:
                        raise Exception("Something weird happen, cycle crossover should be used for permutations")
                    if cycle_start_value == parent1[index_p1]:
                        break
                cycles.append(cycle)
            offspring = numpy.array([parent0, parent1])
            for cycle in cycles:
                for index in cycle:
                    offspring[0][index] = parent1[index]
                    offspring[1][index] = parent0[index]
            if offsprings is None:
                offsprings = offspring
            else:
                numpy.append(offsprings, offspring, axis=0)
            # offsprings.append(offspring[0])
            # offsprings.append(offspring[1])
            offsprings = numpy.unique(offsprings, axis=0)
            if len(offsprings) >= offspring_size[0]:
                break
        return offsprings[offspring_size[0]]
        ...
        for offspring_idx in range(offspring.shape[0]):
            ...
            for gene_idx in mutation_indices:
                ...
                # If the mutation_by_replacement attribute is False, then the random value is added to the gene value.
                    ...
                    offspring[offspring_idx, gene_idx] = self.gene_type(
    ...
    def swap_mutation2(self, offspring):
        ...
        It returns an array of the mutated offspring.
        """
        idxs = random.sample([x for x in range(offspring.shape[0])], 2)
        temp = offspring[idxs[0]]
        offspring[idxs[0]] = offspring[idxs[1]]
        offspring[idxs[1]] = temp
        return offspring
